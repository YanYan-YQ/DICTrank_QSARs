...
def para_selection(var, X, X_test, y, y_test, base_path):
    #base_path = '/Users/Yanyan.Qu/OneDrive - FDA/Desktop/Yanyan/LLM/result/LR/lr'+ var
    #para
    Cs = [0.001, 0.01, 0.1, 1, 10]
    weights=['balanced']
    paras = [l for l in itertools.product(Cs, weights)]
    ...
    for i in range(20):
        ...
        for train_index, validation_index in skf.split(X,np.array(y, dtype=np.int)):
            ...
            ### define column name
            col_name = 'lr_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_K_'+str(para)
            col_name1 = 'lr_'+'seed_'+str(i)+'_paras_'+var+'_K_'+str(para)
            col_name2 = 'lr_'+'paras_'+var
           
    

            ###create classifier
            clf = LogisticRegression(C=para[0], max_iter=300, class_weight =para[1])
...
para_selection(var, X, X_test, y, y_test, base_path+'/lr/lr'+ var)

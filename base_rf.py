...
def para_selection(var, X, X_test, y, y_test, base_path):
    ...
    weight = ['balanced']
    paras = [l for l in itertools.product(n_estimators, max_depth, seeds, min_samples_leaf, weight)]
    ...
    for i in range(20):
        ...
        for train_index, validation_index in skf.split(X,np.array(y, dtype=np.int)):
            ...
            ### define column name
            col_name = 'rf_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_K_'+str(para)
            col_name1 = 'rf_'+'seed_'+str(i)+'_paras_'+var+'_K_'+str(para)
            col_name2 = 'rf_'+'paras_'+var
           
    

            ###create classifier
            clf = RandomForestClassifier(random_state=para[2], n_estimators=para[0], max_depth=para[1],  min_samples_leaf=para[3], class_weight =para[4], bootstrap = True, max_features='log2')
...
para_selection(var, X, X_test, y, y_test, base_path+'/rf/rf'+ var)
